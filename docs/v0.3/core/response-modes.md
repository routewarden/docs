# Response Modes Reference (v0.3.2)

::: warning Legacy Version Notice
You are viewing archived documentation for **v0.3.2**. [Switch to Latest ➔](/traefik/getting-started)
:::

RouteWarden features a modular response engine that dictates exactly how the reverse proxy handles blocked requests. Rather than only offering a generic static 403 error page, RouteWarden supports **13 deterministic response modes** ranging from standard REST error bodies to interactive bot challenges and offensive honeypot traps.
