# Example 6: Kubernetes IngressRoute (Traefik CRD) (v0.3.2)
...
When running Caddy in Kubernetes (via the official Caddy Ingress Controller or custom Caddy DaemonSet/Deployment built with `xcaddy --with github.com/routewarden/caddy-warden@v0.3.2`), manage RouteWarden via a mounted `ConfigMap`:
